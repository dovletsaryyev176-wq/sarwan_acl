...
@permission_required('credits.set_limit')
...
@permission_required('credits.view_payments')
def get_client_credit_payments(client_id):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(*) as total FROM credit_payments WHERE client_credit_id = %s",
                (client_credit_id,)
            )
...
@permission_required('credits.create_payment')
def create_credit_payment():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id, used_credit FROM client_credits WHERE id = %s FOR UPDATE",
                (data['client_credit_id'],)
            )
            credit = cursor.fetchone()
