...
def send_sms():
    ...
    try:
        with conn.cursor() as cursor:
            if phone.lower() in ('all', 'все'):
                cursor.execute("SELECT DISTINCT phone FROM client_phones")
                phones_to_send = [row['phone'] for row in cursor.fetchall() if row.get('phone')]
...
@permission_required('sms.view_history')
def get_sms_history():
    conn = Db.get_connection()
    try:
        with conn.cursor() as cursor:
...
@permission_required('sms.view_detail')
