...
def get_users():
    ...
    try:
        with conn.cursor() as cursor:
            if role_param:
                cursor.execute(
                    "SELECT id, full_name, username, phone, role, is_active FROM users WHERE role=%s",
                    (role_param,)
                )
            else:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if user['role'] == 'courier' and 'full_name' in data:
                cursor.execute(
                    "UPDATE locations SET name=%s WHERE user_id=%s AND type='courier'",
...
@permission_required('users.unblock')
