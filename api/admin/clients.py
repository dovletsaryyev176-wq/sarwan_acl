...
@permission_required('clients.view_one')
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if client['location_id']:
                cursor.execute(
                    "SELECT SUM(quantity) as total FROM stocks WHERE location_id = %s",
                    (client['location_id'],)
                )
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if updates:
                ...
                cursor.execute(f"UPDATE clients SET {', '.join(updates)} WHERE id = %s", tuple(params))
...
@permission_required('clients.toggle_active')
def toggle_client_active(client_id):
    ...
    try:
        ...
        return jsonify({"message": "Статус успешно обновлен", "is_active": target_status}), 200
    finally:
        conn.close()


@admin_bp.route('/clients/<int:client_id>/block-reasons', methods=['GET'])
@permission_required('clients.view_block_reasons')
...
@permission_required('clients.view_phones')
...
@permission_required('clients.add_phone')
...
@permission_required('clients.delete_phone')
...
@permission_required('clients.view_addresses')
    ...
    try:
        with conn.cursor() as cursor:
            ...
            addresses_list = [
                {
                    "id": a['id'],
                    "city_id": a['city_id'], "city_name": a['city_name'] or "Неизвестно",
                    "district_id": a['district_id'], "district_name": a['district_name'] or "Неизвестно",
                    "street_id": a['street_id'], "street_name": a['street_name'] or "Неизвестно",
                    "address_line": a['address_line'],
                    "appartment": a['appartment'],
                    "entrance": a['entrance'],
                    "floor": a['floor'],
...
@permission_required('clients.add_address')
...
@admin_bp.route('/clients/addresses/<int:address_id>', methods=['PATCH'])
@permission_required('clients.update_address')
def update_address(address_id):
    data = request.get_json()
    conn = Db.get_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT id FROM client_addresses WHERE id = %s", (address_id,))
            if not cursor.fetchone():
            ...
            for field in ('city_id', 'district_id', 'address_line', 'appartment', 'entrance', 'floor'):
                if field in data:
                    updates.append(f"{field} = %s")
                    params.append(data[field])

            if 'street_id' in data:
                updates.append("street_id = %s")
                params.append(data['street_id'] or None)

            if not updates:
                return jsonify({"error": "Нет данных для обновления"}), 400

            params.append(address_id)
            cursor.execute(f"UPDATE client_addresses SET {', '.join(updates)} WHERE id = %s", tuple(params))
            conn.commit()
...
@permission_required('clients.delete_address')
