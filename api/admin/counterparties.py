...
@permission_required('counterparties.view')
...
@permission_required('counterparties.create')
def create_counterparty():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("INSERT INTO counterparties (name) VALUES (%s)", (name,))
            counterparty_id = cursor.lastrowid

            cursor.execute(
                "INSERT INTO locations (name, type, counterparty_id) VALUES (%s, 'counterparty', %s)",
                (name, counterparty_id)
            )

            for addr in addresses:
                cursor.execute(
                    "INSERT INTO counterparty_addresses (counterparty_id, address_line) VALUES (%s, %s)",
                    (counterparty_id, addr)
                )

            for ph in phones:
                cursor.execute(
                    "INSERT INTO counterparty_phones (counterparty_id, phone) VALUES (%s, %s)",
                    (counterparty_id, ph)
                )
            ...
            cursor.execute("""
                SELECT c.id, c.name, c.is_active, l.id AS location_id, l.name AS location_name
                FROM counterparties c
                LEFT JOIN locations l ON l.counterparty_id = c.id AND l.type = 'counterparty'
                WHERE c.id = %s
            """, (counterparty_id,))
            cp = cursor.fetchone()
            ...
            result = {
                ...
                "location": {"id": cp["location_id"], "name": cp["location_name"]} if cp["location_id"] else None,
                "addresses": addr_list,
                "phones": phone_list
            }
...
@permission_required('counterparties.update')
def update_counterparty(c_id):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT id FROM counterparties WHERE id = %s", (c_id,))
            if not cursor.fetchone():
                return jsonify({"error": "Контрагент не найден"}), 404

            if name:
                cursor.execute("UPDATE counterparties SET name = %s WHERE id = %s", (name, c_id))
                cursor.execute(
                    "UPDATE locations SET name = %s WHERE counterparty_id = %s AND type = 'counterparty'",
                    (name, c_id)
                )

            if addresses is not None:
                cursor.execute("DELETE FROM counterparty_addresses WHERE counterparty_id = %s", (c_id,))
                for addr in addresses:
                    cursor.execute(
                        "INSERT INTO counterparty_addresses (counterparty_id, address_line) VALUES (%s, %s)",
                        (c_id, addr)
            ...
            if phones is not None:
                cursor.execute("DELETE FROM counterparty_phones WHERE counterparty_id = %s", (c_id,))
                for ph in phones:
                    cursor.execute(
                        "INSERT INTO counterparty_phones (counterparty_id, phone) VALUES (%s, %s)",
                        (c_id, ph)
...
@permission_required('counterparties.block')
...
@permission_required('counterparties.unblock')
