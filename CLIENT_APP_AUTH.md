...
При локальной отладке этой ошибки не будет: в режиме разработки SMS не отправляется,
а код печатается в консоль сервера — см. раздел 9.
...
## 9. Локальный запуск бэкенда

Если бэкенд поднимается на своей машине для отладки.

**Порядок установки**

```bash
python -m venv .venv                 # venv собран на Python 3.14
.venv\Scripts\activate
pip install -r requirements.txt

mysql -u root -p sarwan < дамп_который_вам_дали.sql
...
mkdir uploads                        # git не хранит пустые папки
python app.py                        # waitress на 0.0.0.0:5001
```

Файла `.env` в репозитории нет, он в `.gitignore`. Создайте свой:
...
CLIENT_OTP_DEV_MODE=1
```

**Код из SMS в консоли**

Код хранится в базе в виде необратимого хеша, достать его оттуда нельзя. Поэтому
для локальной работы есть `CLIENT_OTP_DEV_MODE=1`: SMS не отправляется, а код
печатается в консоль, где запущен `app.py`.

```
WARNING [client_app.otp]
============================================================
...
Всё остальное работает как обычно: код кладётся в базу, живёт те же 5 минут,
лимиты на повтор действуют, `/auth/otp/verify` принимает его штатно.

**Код намеренно не возвращается в HTTP-ответе** — только в консоль. Не закладывайтесь
на его получение из ответа `/auth/otp/send`, в бою этого не будет.

На боевом сервере режим не включится: при `FLASK_ENV=production` он принудительно
выключен, что бы ни стояло во флаге.

**Доступ с устройства**

| Откуда | Адрес |
|---|---|
| эмулятор Android | `http://10.0.2.2:5001` |
| симулятор iOS | `http://localhost:5001` |
| реальный телефон | `http://<IP машины в локальной сети>:5001` |

Понадобится разрешить входящий порт 5001 в брандмауэре. Для Android с реальным
устройством — `usesCleartextTraffic` или `network_security_config`, иначе система
не пропустит HTTP.

---

## 10. Проверка на тестовых данных
